...
public class Matrix implements Function {
    ...
    /**
     * creates a matrix based off the given 1D array putting all the arrays values
     * into the first column.
     * 
     * @param arr               the array to base the matrix off of
     * @param orientateAsColumn if you want the array to become a column in the
     *                          matrix as opposed to a row
     */
    public Matrix(double[] arr) {
        double[][] temp = { arr };
        this.matrix = temp;
        ...
        if (columns != m.getRows()) {
            ...
            for (int i = 0; i < newM.rows; i++) {
                for (int j = 0; j < newM.columns; j++) {
                    ...
                    double[] row = new double[a.getColumns()]; // creates an array of length equal to the number of a's
                                                               // columns
                    double[] column = new double[b.getRows()]; // creates an array of length equal to the nymber of b's
                                                               // rows
                    ...
                    // sets the current element of the new matrix to the dot product of the row and
                    // column of a and b;
    ...
    public static Matrix staticMultiply(Matrix m1, Matrix m2) throws Exception {
        ...
        if (m1.getColumns() != m2.getRows()) {
            ...
            throw new Exception(
                    "the first matrices number of columns must be equal to the second matrices number of rows");
            ...
            for (int i = 0; i < newM.rows; i++) {
                for (int j = 0; j < newM.columns; j++) {
                    ...
                    // sets the current element of the new matrix to the dotproduct of the row and
                    // column of a and b;
                    newM.set(i, j, Vector.dotProduct(vRow, vColumn));
